(function ($) {
	...
	window.iSparta.apng ={
		...
		exec:function(id){
			...
			function dirHandle(){
				...
				var urlsPromises = urls.map(function(url,i){
					return new Promise(function(resolve, reject) {
						tempindex++;
						var tempPath = tempdir+'apng'+(i+1)+'.png';
			...
			// refactor lossless png to apng
			function losslessPng2apngExec() {
				losslessPngoptExec().then(function() {
					return losslessApngasmExec();
				}).then(function() {
					return losslessApngoptExec();
				}).then(function() {
					window.iSparta.apng.switch(id+1);
				}).catch(function(err) {
					console.log(err);
			...
			function losslessPngoptExec() {
				// sequential to process to reduce CPU usage
				var finalPromise = originPngPaths.reduce(function(previousPromise, pngPath, i) {
					return previousPromise.then(function() {
						return losslessPngoutExec(pngPath,i);
					});
				}, Promise.resolve());
			...
			function losslessPngoutExec(pngPath,i) {
			...
			// png to apng asm
			function losslessApngasmExec() {
				var inputPath = tempdir+'frame*.png';
				var outputPath = tempdir+'apngout.png';
				var apngasmcomd = '"'+apngasm+'" "'+outputPath+'" "'+inputPath+'" '+rate*100+' 100'+' -l'+loop+'';
				return new Promise(function(resolve, reject) {
